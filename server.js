...
app.post('/login', (req,res)=>{
  let u=req.body.username; let p=req.body.password;
  if((u==='admin'&&p==='admin')||(u==='Remix22'&&p==='2212')){
    req.session.logged=true; req.session.username=u; res.redirect('/dashboard');
  } else res.render('login', {error: 'Usuario o clave incorrecta'});
});
...
function crear(req,res){
  ...
  let f=new Date(); if(tipo==='Demo') f.setHours(f.getHours()+1); else f.setDate(f.getDate()+30);
...
app.post('/api/renew-user', checkAuth, (req,res)=>{
  ...
  if(u){ let f=new Date(); f.setDate(f.getDate()+30); u.vencimiento=f.toLocaleString('es-AR'); u.vencimiento_ms=f.getTime(); u.tipo='Premium'; db.credits=Math.max(0,db.credits-1); save(); res.json({ok:true}); } else res.json({ok:false});
});
